...
// distance i-j in pbc
  ...
  walker[iv] = 4.0 * v + vtail*npart; // Potential energy
  walker[ik] = kin; // Kinetic energy
  walker[it] = (2.0 / 3.0) * kin/(double)npart; // Temperature
  walker[ie] = 4.0 * v + kin;  // Total energy;
  walker[ip] = rho*walker[it] + (1./(3.*vol))*(viriale + 3.*npart*ptail);   //Pressure
...
void Averages(int iblk) //Print results for current block
{
   ...
   ofstream Epot;//, Ekin, Etot, Temp, Pres;
    ...
    /*
    Ekin.open("solid/solid_ekin.dat",ios::app);
    Temp.open("solid/solid_temp.dat",ios::app);
    Etot.open("solid/solid_etot.dat",ios::app);
    Pres.open("solid/solid_pres.dat",ios::app);
    */
