...
if(in.fail()) {
			...
			cerr<<"problemi di apertura del canale"<<endl;
			exit(1);
		}
...
for(int i=0; i<50; ++i) {
    ...
    in >> posizione[i].x >> posizione[i].y;
...
void genetic::Print(int myrank) {

    ofstream pos, ave;
    pos.open("square." + to_string(myrank) ,ios::app);
    ave.open("L." + to_string(myrank) ,ios::app);
    int m;
